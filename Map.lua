...
function Map:init()
    ...
    while x < self.mapWidth do
        ...
        if x > self.mapWidth - 2 then
            ...
            self:setTile(x + 1, self.mapHeight / 2 - 3, FLAG, TILE_EMPTY)
            x = self.mapWidth
        end
...
function Map:render()
    ...
    -- when flag is reached
    if gameState == 'done' then
        love.graphics.setDefaultFilter('nearest', 'nearest')
        love.graphics.setFont(love.graphics.newFont('fonts/font.ttf', 20))
        love.graphics.clear(40/255, 45/255, 52/255, 255/255)
        love.graphics.printf("You Have Won!", 0, VIRTUAL_HEIGHT/2, VIRTUAL_WIDTH+120, 'center')
        self.music:stop()
